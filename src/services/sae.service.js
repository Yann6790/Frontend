...
export const saeService = {
  ...
  async getAnnouncementById(saeId, annId) {
    const res = await apiClient(`/api/saes/${saeId}/announcements/${annId}`, { method: 'GET' });
    return res.data || res;
  },
  ...
  async updateSubmissionVisibility(saeId, submissionId, isPublic) {
    const res = await apiClient(`/api/saes/${saeId}/submissions/${submissionId}/visibility`, {
      method: 'PATCH',
      body: JSON.stringify({ isPublic }),
    });
    return res.data || res;
  ...
  async getGradeCategories(saeId) {
    const res = await apiClient(`/api/saes/${saeId}/grade-categories`, { method: 'GET' });
    return res.data || res;
  },
  ...
  async exportGrades(saeId) {
    const res = await apiClient(`/api/saes/${saeId}/grades/export`, { method: 'GET' });
  ...
  async getSubmissionGrades(submissionId) {
    const res = await apiClient(`/api/submissions/${submissionId}/grades`, { method: 'GET' });
    return res.data || res;
  },
  ...
  async updateMilestone(saeId, milestoneId, data) {
    const res = await apiClient(`/api/saes/${saeId}/milestones/${milestoneId}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    });
    return res.data || res;
  ...
  async getMilestoneStats(saeId) {
    const res = await apiClient(`/api/saes/${saeId}/milestones/stats`, { method: 'GET' });
    return res.data || res;
  },
  ...
  async getMilestoneProgressForStudent(saeId, milestoneId, studentId) {
    const res = await apiClient(`/api/saes/${saeId}/milestones/${milestoneId}/progress/${studentId}`, { method: 'GET' });
    return res.data || res;
  },
  ...
  async getAllPublicSubmissions() {
    ...
    try {
      ...
      archiveItems = Array.isArray(archivesRes) ? archivesRes : (archivesRes?.data || []);
    } catch (err) {
      console.warn('[Gallery] Impossible de charger les archives:', err.message);
    }
    ...
    try {
      ...
      const saes = Array.isArray(saesRes) ? saesRes : (saesRes?.data || saesRes?.items || []);
      ...
      results.forEach(result => {
        ...
        const subsArray = Array.isArray(subs) ? subs : (subs?.items || subs?.data || []);
        ...
        subsArray
          ...
          .forEach(sub => {
            ...
            liveItems.push({
              ...
              year:        new Date(sub.submittedAt || sae.dueDate || sae.createdAt || Date.now()).getFullYear(),
            });
            archiveIds.add(sub.id);
          });
      });
    } catch (err) {
      console.warn('[Gallery] Impossible de charger les soumissions live:', err.message);
    }
  ...
  async getAllAnnouncements() {
    try {
      ...
      console.warn('[Announcements] Erreur lors de l\'agrégation globale:', err.message);
      return [];
    }
  },
